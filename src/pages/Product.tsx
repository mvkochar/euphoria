    ...
    return (
        <>
            <main className='product-main d-f align-center'>
                ...
                <div>
                    ...
                    <div className="colors-box d-f align-center">
                        <div className={color === 1 ? "colors-item-outer outer-active" : "colors-item-outer" }>
                            <div className="colors-item" onClick={() => setColor(1)}></div>
                        </div>
                        <div className={color === 2 ? "colors-item-outer outer-active" : "colors-item-outer"}>
                            <div className="colors-item" onClick={() => setColor(2)}></div>
                        </div>
                        <div className={color === 3 ? "colors-item-outer outer-active" : "colors-item-outer"}>
                            <div className="colors-item" onClick={() => setColor(3)}></div>
                        </div>
                        <div className={color === 4 ? "colors-item-outer outer-active" : "colors-item-outer"}>
                            <div className="colors-item" onClick={() => setColor(4)}></div>
                        </div>
                    </div>
                    <div className="product-main-action d-f">
                        <button type="button" className='product-main-btn' onClick={addProductToCart}>Add to cart</button>
            ...
            <section className="product-similar">
                <h2 className="home-bl-title">Similar Products</h2>
                <div className="product-similar-box d-f jc-sb">
                    {
                        ProductsList.filter((elem) => elem.id !== index).filter((elem) => elem.id < 9).map((product) => {
                            return (
                                <ProductsItem
                                    id={product.id}
                                    image={product.image}
